...
(defvar canvas--bencode-max-depth 100 "Maximum nesting depth for bencode decoding.")
(defvar canvas--bencode-max-string (* 64 1024 1024) "Maximum bencode string length (64MB).")

(defun canvas--bencode-decode-from (str pos &optional depth)
  "Decode bencode value from STR starting at POS. Return (value . new-pos)."
  (let ((depth (or depth 0)))
    (when (> depth canvas--bencode-max-depth)
      (error "Bencode nesting too deep (>%d)" canvas--bencode-max-depth))
    (when (>= pos (length str))
      ...
      (cond
       ...
       ((= ch ?l)
        (let ((items nil) (p (1+ pos)))
          (while (and (< p (length str)) (/= (aref str p) ?e))
            (let ((result (canvas--bencode-decode-from str p (1+ depth))))
       ...
       ((= ch ?d)
        (let ((dict nil) (p (1+ pos)))
          (while (and (< p (length str)) (/= (aref str p) ?e))
            (let* ((key-result (canvas--bencode-decode-from str p (1+ depth)))
                   (val-result (canvas--bencode-decode-from str (cdr key-result) (1+ depth))))
       ...
       ((and (>= ch ?0) (<= ch ?9))
        (let* ((colon (cl-position ?: str :start pos))
               ...
               (start (1+ colon))
               (end (+ start len)))
          (when (< len 0) (error "Negative bencode string length"))
          (when (> len canvas--bencode-max-string) (error "Bencode string too large: %d" len))
